...
struct EQBandParam { LONG bandIndex; FLOAT gainLinear; };

// ===== 宏 & 小工具 =====
#define HR_OK(hr)   (SUCCEEDED((hr)))
#define HR_FAIL(hr,msg) wprintf(L"[!] %s (hr=0x%08X)\n", L##msg, (hr))
#define RETURN_IF_FAILED(hr, msg) do{ if(FAILED(hr)){ HR_FAIL(hr,msg); goto done; } }while(0)
...
static HRESULT GetInterfacePath(IMMDevice* endpoint, std::wstring& ifPath) {
    ...
    if (FAILED(hr) || cc == 0) { topoEp->Release(); return FAILED(hr) ? hr : E_FAIL; }

    IConnector* epConn = nullptr; hr = topoEp->GetConnector(0, &epConn);
    if (FAILED(hr)) { topoEp->Release(); return hr; }

    IConnector* devConn = nullptr; hr = epConn->GetConnectedTo(&devConn);
    if (FAILED(hr)) { epConn->Release(); topoEp->Release(); return hr; }

    // 设备侧 Connector -> Part -> TopologyObject -> GetDeviceId() 得到接口路径
    IPart* part = nullptr; hr = devConn->QueryInterface(__uuidof(IPart), (void**)&part);
    if (FAILED(hr)) { devConn->Release(); epConn->Release(); topoEp->Release(); return hr; }

    IDeviceTopology* topoDev = nullptr; hr = part->GetTopologyObject(&topoDev);
    if (FAILED(hr)) { part->Release(); devConn->Release(); epConn->Release(); topoEp->Release(); return hr; }

    LPWSTR devPath = nullptr; hr = topoDev->GetDeviceId(&devPath); // 注意：这是 KS 接口路径
...
static HRESULT GetHardwareIdFromInterfacePath(const std::wstring& ifPath, std::wstring& hwid) {
    ...
    if (hdi == INVALID_HANDLE_VALUE) return HRESULT_FROM_WIN32(GetLastError());

    SP_DEVICE_INTERFACE_DATA ifData{};
    ifData.cbSize = sizeof(ifData);
    if (!SetupDiOpenDeviceInterfaceW(hdi, ifPath.c_str(), 0, &ifData)) {
        DWORD err = GetLastError(); SetupDiDestroyDeviceInfoList(hdi);
...
static void ListEndpoints(Flow flow) {
    ...
    if (FAILED(CoCreateInstance(__uuidof(MMDeviceEnumerator), nullptr, CLSCTX_ALL,
                                __uuidof(IMMDeviceEnumerator), (void**)&e))) return;
    EDataFlow df = (flow == Flow::Render) ? eRender : eCapture;
    if (FAILED(e->EnumAudioEndpoints(df, DEVICE_STATE_ACTIVE, &col))) { e->Release(); return; }

    UINT n=0; if (FAILED(col->GetCount(&n))) { col->Release(); e->Release(); return; }
    wprintf(L"--- %s endpoints ---\n", (flow==Flow::Render)?L"Render":L"Capture");
    for (UINT i=0;i<n;++i) {
        IMMDevice* ep=nullptr; if (FAILED(col->Item(i,&ep))) continue;
        ...
        if (SUCCEEDED(ep->OpenPropertyStore(STGM_READ,&ps))) {
            if (SUCCEEDED(ps->GetValue(PKEY_Device_FriendlyName,&v)) && v.pwszVal) name=v.pwszVal;
            PropVariantClear(&v); ps->Release();
        }
        GetInterfacePath(ep, ifPath);
        if (!ifPath.empty()) GetHardwareIdFromInterfacePath(ifPath, hwid);
...
static HRESULT FindEndpoint(Flow flow,
    ...
    *ppDev = nullptr;
    ...
    if (FAILED(hr)) { e->Release(); return hr; }

    UINT n = 0; hr = col->GetCount(&n);
    if (FAILED(hr)) { col->Release(); e->Release(); return hr; }

    if (index >= 0 && (UINT)index < n) {
        hr = col->Item(index, ppDev);
        if (SUCCEEDED(hr) && pIfPath) { std::wstring ifp; GetInterfacePath(*ppDev, ifp); *pIfPath = ifp; }
        col->Release(); e->Release();
        return hr;
    }
    ...
    for (UINT i = 0; i < n; ++i) {
        ...
        if (!key.empty()) {
            std::wstring ifp; if (SUCCEEDED(GetInterfacePath(ep, ifp)) && !ifp.empty()) {
                std::wstring hwid; GetHardwareIdFromInterfacePath(ifp, hwid);
                if (!hwid.empty()) {
                    if (NormalizeId(hwid).find(key) != std::wstring::npos) ok = true;
                }
                if (!ok) {
                    if (NormalizeId(ifp).find(key) != std::wstring::npos) ok = true;
                }
            }
        }
        ...
        if (!ok && !nameKey.empty()) {
            ...
            if (SUCCEEDED(ep->OpenPropertyStore(STGM_READ,&ps))) {
                if (SUCCEEDED(ps->GetValue(PKEY_Device_FriendlyName,&v)) && v.pwszVal) {
                    if (ToUpper(v.pwszVal).find(nameKey) != std::wstring::npos) ok = true;
                }
                PropVariantClear(&v); ps->Release();
            }
        }
        ...
        // 都没给筛选条件时，取第一项
        if (!ok && key.empty() && nameKey.empty()) ok = true;

        if (ok) {
            if (pIfPath) { std::wstring ifp; GetInterfacePath(ep, ifp); *pIfPath = ifp; }
            *ppDev = ep; col->Release(); e->Release();
            return S_OK;
        }
        ep->Release();
    }
...
static HRESULT GetKsControl(IMMDevice* dev, IKsControl** ppKs) {
    *ppKs = nullptr;
    ...
    // 1) IAudioClient 路径
    {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr) && mix) {
                ...
                fmt.nSamplesPerSec=16000; fmt.wBitsPerSample=16;
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(hr)) { if (mix) CoTaskMemFree(mix); ac->Release(); wprintf(L"[+] IKsControl via IAudioClient\n"); return S_OK; }
            }
    ...
    {
        IDeviceTopology* topo=nullptr; hr = dev->Activate(__uuidof(IDeviceTopology), CLSCTX_ALL, nullptr, (void**)&topo);
        if (SUCCEEDED(hr)) {
            UINT cc=0; hr = topo->GetConnectorCount(&cc);
            if (SUCCEEDED(hr) && cc>0) {
                IConnector* epConn=nullptr; if (SUCCEEDED(topo->GetConnector(0,&epConn))) {
                    IConnector* devConn=nullptr; if (SUCCEEDED(epConn->GetConnectedTo(&devConn))) {
                        IPart* part=nullptr; if (SUCCEEDED(devConn->QueryInterface(__uuidof(IPart),(void**)&part))) {
                            // 关键：很多设备只在 Part 上能 Activate 到 IKsControl
                            hr = part->Activate(CLSCTX_ALL, __uuidof(IKsControl), (void**)ppKs);
                            part->Release();
                            if (SUCCEEDED(hr) && *ppKs) { devConn->Release(); epConn->Release(); topo->Release(); wprintf(L"[+] IKsControl via DeviceTopology Part::Activate\n"); return S_OK; }
                        }
                        // 3) 再在设备侧 Connector 上尝试 QI(IKsControl) 兜底
                        if (FAILED(hr)) {
                            hr = devConn->QueryInterface(__uuidof(IKsControl),(void**)ppKs);
                            if (SUCCEEDED(hr) && *ppKs) { devConn->Release(); epConn->Release(); topo->Release(); wprintf(L"[+] IKsControl via DeviceTopology Connector::QI\n"); return S_OK; }
                        }
                        devConn->Release();
                    }
                    epConn->Release();
...
static HRESULT KsSet(IKsControl* ks, ULONG pid, const T& data) {
    KSPROPERTY prop={}; prop.Set=MYCOMPANY_APO_PROPSETID; prop.Id=pid; prop.Flags=KSPROPERTY_TYPE_SET;
    ULONG ret=0; return ks->KsProperty(&prop,sizeof(prop),(PVOID)&data,sizeof(T),&ret);
}
template<typename T>
static HRESULT KsGet(IKsControl* ks, ULONG pid, T& out) {
    KSPROPERTY prop={}; prop.Set=MYCOMPANY_APO_PROPSETID; prop.Id=pid; prop.Flags=KSPROPERTY_TYPE_GET;
    ULONG ret=0; return ks->KsProperty(&prop,sizeof(prop),(PVOID)&out,sizeof(T),&ret);
}

static bool HexToBytes(const std::wstring& hex, std::vector<BYTE>& out) {
    if (hex.size()%2) return false;
    out.clear(); out.reserve(hex.size()/2);
    for(size_t i=0;i<hex.size();i+=2){ wchar_t b[3]={hex[i],hex[i+1],0}; out.push_back((BYTE)wcstol(b,nullptr,16)); }
    return true;
}

// ---- 选项解析 ----
enum class FlowSel { Render, Capture };
struct Options {
...
static Options Parse(int argc, wchar_t** argv){
    ...
    while(o.argi<argc && wcsncmp(argv[o.argi],L"--",2)==0){
        ...
        if (s==L"--capture") o.flow=FlowSel::Capture;
        else if (s==L"--render") o.flow=FlowSel::Render;
        else if (s==L"--pnp" && o.argi+1<argc) o.pnp=argv[++o.argi];
        else if (s==L"--name"&& o.argi+1<argc) o.name=argv[++o.argi];
        else if (s==L"--index"&&o.argi+1<argc) o.index=_wtoi(argv[++o.argi]);
        else if (s==L"--verbose") o.verbose=true;
        else break;
        ++o.argi;
    }
    return o;
...
int wmain(int argc, wchar_t** argv) {
    ...
    if (FAILED(hr)) { HR_FAIL(hr, "CoInitializeEx"); return 1; }

    Options opt = Parse(argc, argv);
    if (opt.argi >= argc) { wprintf(L"[!] 缺少命令；使用 list/gain/eq/... \n"); CoUninitialize(); return 0; }
    ...
    if (opt.verbose) {
        std::wstring hwid; GetHardwareIdFromInterfacePath(ifPath, hwid);
    ...
    {   // 命令处理放内层作用域，避免 goto 跨越初始化
        ...
        if (cmd == L"gain" && opt.argi+1 < argc) {
            ...
            KSPROPERTY prop={}; prop.Set=MYCOMPANY_APO_PROPSETID; prop.Id=PID_ParamsBlob; prop.Flags=KSPROPERTY_TYPE_SET;
            ULONG ret=0; hr = ks->KsProperty(&prop,sizeof(prop),bytes.data(),(ULONG)bytes.size(),&ret);
            HR_OK(hr) ? wprintf(L"[OK] Set Blob (%u bytes)\n", (UINT)bytes.size())
                      : HR_FAIL(hr, "Set Blob");
        }
        else if (cmd == L"get" && opt.argi+1 < argc) {
            std::wstring which = argv[opt.argi+1]; std::transform(which.begin(),which.end(),which.begin(),::towlower);
            if (which == L"gain")     { float g=0; hr=KsGet(ks,PID_Gain,g);     HR_OK(hr)?wprintf(L"[OK] Gain=%f\n",g):HR_FAIL(hr,"Get Gain"); }
            else if (which==L"reverb"){ float m=0; hr=KsGet(ks,PID_Reverb,m);  HR_OK(hr)?wprintf(L"[OK] Reverb=%f\n",m):HR_FAIL(hr,"Get Reverb"); }
            else if (which==L"limiter"){ float t=0; hr=KsGet(ks,PID_Limiter,t); HR_OK(hr)?wprintf(L"[OK] Limiter=%f\n",t):HR_FAIL(hr,"Get Limiter");}
            else wprintf(L"[!] 未实现 get %s\n", which.c_str());
        }
        else {
